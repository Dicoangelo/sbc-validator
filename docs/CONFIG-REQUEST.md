...
**Why this is the unlock.** The validator is built end-to-end on four vendors,
...
Ribbon, and Oracle they stay deliberately **silent** (no false positives) until
we see a real export. One real config per vendor turns those domains on for that
vendor and hardens the parser against production grammar. **Even one config (any
vendor) is high value.**
